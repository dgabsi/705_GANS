...
def calc_losses_dcgan(batch_real, generator, discriminator, optim_generator, optim_discr, device):
    ...
    noise_for_discr = generator.module.generate_noise(batch_real.size()[0]).to(device)
    ...
    noise_gener = generator.module.generate_noise(batch_real.size()[0]).to(device)
...
def calc_losses_wsgan(batch_real, generator, critic, optim_generator, optim_critic, device,num_iter_critic=5, lambda_penalty=10):
    ...
    for iter in (range(num_iter_critic)):
        ...
        noise_for_critic = generator.module.generate_noise(batch_size).to(device)
        fake_for_critic = generator(noise_for_critic).to(device)

        # critic on fake
        critic_fake_pred = critic(fake_for_critic.detach()).view(-1)
        # critic on real
        critic_real_pred = critic(batch_real).view(-1)

        # Gradient penalty-Enforcing 1 Lifshitz continuous https://arxiv.org/abs/1704.00028
        channels, height,  width = batch_real.size()[1:]
        epsilon_mix = torch.rand((batch_size, 1, 1, 1)).repeat(1, channels, height,  width).to(device)
        mixed_images = batch_real * epsilon_mix + fake_for_critic * (1 - epsilon_mix)
        critic_scores_mixed = critic(mixed_images)
        grad_mixed = torch.autograd.grad(inputs=mixed_images,
                                         outputs=critic_scores_mixed,
                                         grad_outputs=torch.ones_like(critic_scores_mixed),
                                         create_graph=True,
                                         retain_graph=True
                                         )[0]
        norm_grad_mixed = grad_mixed.flatten(1).norm(2, dim=1)
        grad_penalty = lambda_penalty * criterion(norm_grad_mixed, torch.ones_like(norm_grad_mixed))
    ...
    # Generator loss
    optim_generator.zero_grad()
    noise_gener = generator.module.generate_noise(batch_size).to(device)
    fake = generator(noise_gener).to(device)
...
def calc_losses_sagan(batch_real, generator, discriminator, optim_generator, optim_discr, device):
    ...
    discr_fake_pred = discriminator(fake_for_discr.detach()).view(-1)

    #Dicrimnator loss is the Hinge loss of real image and fake images
    disc_fake_loss = torch.nn.ReLU()(1.0 + discr_fake_pred).mean()
    disc_real_pred = discriminator(batch_real).view(-1)
    disc_real_loss = torch.nn.ReLU()(1.0 - disc_real_pred).mean()
...
def calc_losses_srgan(batch_real_hq, batch_real_lq, generator, discriminator, optim_generator, optim_discr, vgg_output, device):
    ##The SRGAN according to https://arxiv.org/abs/1609.04802, https://arxiv.org/pdf/1903.09922.pdf
    vgg_output=vgg_output.to(device)
    batch_real_lq=batch_real_lq.to(device)
    ...
    #Discriminator loss
    batch_hq_fake=generator(batch_real_lq).to(device)
    ...
    noise_fake_discr = torch.rand_like(discr_fake_hq_pred) * 0.1
    noise_real_discr = torch.rand_like(discr_real_hq_pred) * 0.1
    ...
    #Generator loss
    optim_generator.zero_grad()
    generator_fake_pred = discriminator(batch_hq_fake.detach())
...
def train_gan(num_epochs, batch_size, device, train_dataset, val_dataset, generator, discriminator,  config_optim, type='DCGAN',run_dir=None, saved_dir=None, run_name='', verbose_show=50, calc_IS=True):
    ...
    generator_optimizer = optim.Adam(generator.parameters(), **config_optim['optim_gen'])
    discriminator_optimizer = optim.Adam(discriminator.parameters(), **config_optim['optim_disc'])
    global_step =0
    ...
    if type=='SRGAN':
        ...
        noise_for_tesorboard = generator.module.generate_noise(num_images_tensorboard).to(device)
    generator.train()
    discriminator.train()
    ...
    for epoch in range(num_epochs):
        for num_batch, data in enumerate(data_loader):
            ...
            if type=='DCGAN':
                gener_loss,  discr_loss, batch_accuracy_real, batch_accuracy_fake=calc_losses_dcgan(batch_real, generator, discriminator, generator_optimizer, discriminator_optimizer, device)
            ...
            if type=='WSGAN':
                gener_loss,  discr_loss=calc_losses_wsgan(batch_real, generator, discriminator, generator_optimizer, discriminator_optimizer, device)
                if not num_batch % verbose_show:
                    print(f"Batch{num_batch}/Epoch: {epoch} Critic: {discr_loss:.3f} Generator loss: {gener_loss:.3f} ")

            if type=='SAGAN':
                gener_loss, discr_loss, batch_accuracy_real, batch_accuracy_fake = calc_losses_sagan(batch_real,  generator,  discriminator,  generator_optimizer,  discriminator_optimizer, device)
                if not num_batch % verbose_show:
                    print(
                    f"Batch{num_batch}/Epoch {epoch} Discriminator: {discr_loss:.3f} Generator loss: {gener_loss:.3f}  Acc real:{batch_accuracy_real:.3f} Acc fake:{batch_accuracy_fake:.3f}")
                all_real_accuracy.append(batch_accuracy_real)
                all_fake_accuracy.append(batch_accuracy_fake)
            if type=='SRGAN':
                gener_loss, discr_loss= calc_losses_srgan(batch_real , data[1],discriminator,generator_optimizer, discriminator_optimizer, vgg,device)
                if not num_batch % verbose_show:
                    print(
            ...
            with torch.no_grad():
                if not num_batch % 100:
                    ...
                    fake_images_display=fake_images.detach().cpu()
        ...
        inception_FID_score=calc_inception_FID_score(batch_size, device, val_dataset, generator, type)
        inception_FID_scores.append(inception_FID_score)
        if calc_IS:
            inception_score = calc_inception_score(device=device, generator=generator, eval_size=len(val_dataset))
        ...
        discr_filename=f"{type}_Discriminator_epoch_{epoch}"
        discriminator.module.save_checkpoint(saved_dir, discr_filename)
        gener_filename = f"{type}_Generator_epoch_{epoch}"
        generator.module.save_checkpoint(saved_dir, gener_filename)

    plot_all_losses(all_gener_losses, all_discr_losses, f"{type} GAN_losses", saved_dir, f"{type}_gan_losses",'batches')
    #if ((type=='DCGAN') or (type=='SAGAN')):
    #    plot_all_accuracies(all_fake_accuracy, all_real_accuracy, f"{type} GAN_accuracy", saved_dir, f"{type}_gan_accuracy",'batches')
    plot_inception_scores([inception_FID_scores],[f"{type}_FID"], f"{type} GAN_FID", saved_dir, f"{type}_gan_FID", "FID")
    if calc_IS:
        plot_inception_scores([inception_scores], [f"{type}_IS"], f"{type} GAN_Inception_score", saved_dir, f"{type}_gan_IS", "IS")
